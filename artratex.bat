...
@rem ->> Check tools
@rem ------------------------------------------------
where %CompileName% >nul 2>nul
if errorlevel 1 (
echo Error: %CompileName% was not found in PATH.
echo Install TeX Live or MiKTeX first, then reopen the terminal/VS Code.
exit /b 1
)
where bibtex >nul 2>nul
if errorlevel 1 (
echo Error: bibtex was not found in PATH.
echo Install TeX Live or MiKTeX first, then reopen the terminal/VS Code.
exit /b 1
)
@rem ------------------------------------------------
...
@rem ------------------------------------------------
%CompileName% -interaction=nonstopmode -file-line-error -output-directory=Tmp %FileName%
@rem ------------------------------------------------
